...
class MapViewController: UIViewController, MKMapViewDelegate, NSFetchedResultsControllerDelegate {
    ...
    //MARK:- LifeCycle
        override func viewDidLoad() {
            ...
            mapView.addAnnotations(mapAnnotationsFromPins(pins: fetchedResultsController.fetchedObjects!))
        }
...
{            let gesture = sender as? UILongPressGestureRecognizer
            ...
            if(gesture?.state == UILongPressGestureRecognizer.State.began){
                if let location = gesture?.location(in: mapView){
    ...
    extension MapViewController {
        ...
        func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
            ...
            case .insert:
                ...
                mapView.addAnnotation(annotationFromPin(latitude: pin.latitude, longitude: pin.longitude))
                break
            default:
                break
            }
